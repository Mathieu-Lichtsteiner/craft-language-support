...
const show = { prefix: "show", body: "{{ $1 }}", description: "{{ }}" };
const execute = { prefix: "execute", body: "{% $1 %}", description: "{% %}" };
const autoescape = {
  prefix: "autoescape",
  ...
  function registerDocType(type) {
    ...
    if (config.formatting === true) {
      ...
      context.subscriptions.push(
        vscode__default[
          "default"
        ].languages.registerDocumentRangeFormattingEditProvider(type, {
          provideDocumentRangeFormattingEdits: function (document, range) {
            ...
            const rng = new vscode__default["default"].Range(
              new vscode__default["default"].Position(range.start.line, 0),
              end
            );
            return prettyDiff(document, rng);
          },
